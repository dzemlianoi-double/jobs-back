ActiveAdmin.register Vacancy do
  ...
  permit_params :title, :country, :city, :salary_min, :arrive_date, :video_url, :offers_quantity, :age_min, :age_max,
                :is_hot, :is_on_main, :active, :info, :sex, :experience, :position, speciality_vacancies_ids: []
  ...
  show do
    attributes_table do
      %i[country city arrive_date is_hot is_on_main sex title experience salary_min age active info video_url].each do |field|
        row(field)
      end
