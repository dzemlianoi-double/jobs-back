ActiveAdmin.register Service do
  ...
  permit_params %i[name full_description position price active position icon by_agreement]
  ...
  form do |f|
    inputs 'Создать Услугу' do
      f.semantic_errors
      %i[name price active full_description by_agreement position icon].each { |field| f.input(field) }
  ...
  show do
    attributes_table do
      %i[name price active full_description by_agreement position].each { |field| row(field) }
