ActiveAdmin.register Service do
  ...
  permit_params %i[name full_description short_description position price active icon]
  ...
  form do |f|
    inputs 'Создать Услугу' do
      f.semantic_errors
      %i[name price active full_description short_description icon].each { |field| f.input(field) }
