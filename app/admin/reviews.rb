ActiveAdmin.register Review do
  ...
  permit_params %i[first_name last_name avatar text position position active]
  ...
  form do |f|
    inputs 'Создать отзыв' do
      f.semantic_errors
      %i[first_name last_name active position text].each { |field| f.input(field) }
  ...
  show do
    attributes_table do
      %i[first_name last_name position active text].each { |field| row(field) }
